...
export default function AuditPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-semibold text-dark-800">Logs de Auditoria</h1>
        <p className="text-dark-400 mt-1">Registro completo de todas as ações do sistema</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Total', value: stats?.total || 0, icon: ScrollText, color: 'bg-dark-500' },
        ].map((stat, i) => (
      ...
      {stats && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="card">
            <div className="px-5 py-3 border-b border-dark-200">
              <h3 className="text-sm font-semibold text-dark-700">Ações Mais Frequentes (Mês)</h3>
            </div>
            <div className="divide-y divide-dark-100">
              {stats.byAction?.map((item: any) => (
                <div key={item.action} className="px-5 py-2.5 flex items-center justify-between">
                  <span className="text-sm text-dark-700">{actionLabels[item.action] || item.action}</span>
                  <span className="text-sm font-semibold text-dark-800">{item.count}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="card">
            <div className="px-5 py-3 border-b border-dark-200">
              <h3 className="text-sm font-semibold text-dark-700">Usuários Mais Ativos (Mês)</h3>
            </div>
            <div className="divide-y divide-dark-100">
              {stats.byUser?.map((item: any) => (
                <div key={item.userId} className="px-5 py-2.5 flex items-center justify-between">
                  <span className="text-sm text-dark-700">{item.email}</span>
                  <span className="text-sm font-semibold text-dark-800">{item.count} ações</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Logs Table */}
      <div className="card">
        <div className="p-4 border-b border-dark-200 flex flex-wrap gap-3 items-end">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-dark-200 bg-dark-50">
                    <th className="text-left px-4 py-3 text-xs font-semibold text-dark-400 uppercase">Data/Hora</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-dark-400 uppercase">Usuário</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-dark-400 uppercase">Ação</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-dark-400 uppercase">Entidade</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-dark-400 uppercase">IP</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-dark-400 uppercase">Detalhes</th>
                  </tr>
                </thead>
                  ...
                  {logsData?.data.map((log) => (
                    <tr key={log.id} className="hover:bg-dark-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <p className="text-sm font-medium text-dark-800">{log.user?.name}</p>
                        <p className="text-xs text-dark-400">{log.userEmail}</p>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${actionColors[log.action] || 'bg-dark-100 text-dark-700'}`}>
                          {actionLabels[log.action] || log.action}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <p className="text-sm text-dark-500">{log.entityType}</p>
                        {log.entityId && <p className="text-xs text-dark-300 font-mono">{log.entityId.slice(0, 8)}...</p>}
                      </td>
                      <td className="px-4 py-3 text-xs text-dark-400 font-mono">{log.ipAddress}</td>
                      <td className="px-4 py-3">
                        {log.details && Object.keys(log.details).length > 0 && (
                          <details className="text-xs">
                            <summary className="cursor-pointer text-primary-600 hover:text-primary-700">Ver</summary>
                            <pre className="mt-1 p-2 bg-dark-50 rounded text-[10px] max-w-xs overflow-auto">
